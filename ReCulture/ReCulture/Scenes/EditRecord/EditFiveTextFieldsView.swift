//
//  EditFiveTextFieldsView.swift
//  ReCulture
//
//  Created by Suyeon Hwang on 9/20/24.
//

import UIKit

protocol FiveTextFieldsDelegate: AnyObject {
    /// 기록 상세 내용을 작성하는 뷰의 '간단한 후기' 내용이 모두 채워졌을 때 호출되는 메소드
    func shortReviewIsSet()
}

/// 기록 중 스포츠, 콘서트, 독서를 위한 뷰입니다.
final class EditFiveTextFieldsView: UIView {
...
//    private let parentVC: AddRecordDetailVC
    ...
    var shortReviewIsSet: Bool = false {
        didSet {
            delegate?.shortReviewIsSet()
//            parentVC.validateInputField()
        }
    }
    
    weak var delegate: FiveTextFieldsDelegate?
...
//    }
    ...
    func configure(_ placeholderInfoList: [String], withData: [String]) {
        print(placeholderInfoList)
        label1.text = placeholderInfoList[0]
//        textField1.placeholder = placeholderInfoList[0][1]
        textField1.text = withData[0]
        
        label2.text = placeholderInfoList[1]
//        textField2.placeholder = placeholderInfoList[1][1]
        textField2.text = withData[1]
        
        label3.text = placeholderInfoList[2]
//        textField3.placeholder = placeholderInfoList[2][1]
        textField3.text = withData[2]
        
        label4.text = placeholderInfoList[3]
//        textField4.placeholder = placeholderInfoList[3]
        textField4.text = withData[3]
        
        textView5.text = withData[4]
        textView5.textColor = .rcGray600
    }
...
extension EditFiveTextFieldsView: UITextViewDelegate {
